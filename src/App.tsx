...
function SiteHeader() {
  ...
  const [theme, setTheme] = useState<'dark' | 'light'>(() => {
    ...
    return (window.localStorage.getItem('tjyz-theme') as 'dark' | 'light') || 'dark'
  })
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem('tjyz-theme', theme)
  }, [theme])
  ...
  return (
    <header className={`site-header ${aboutTheme ? 'site-header--about' : ''}`}>
      ...
      {!aboutTheme && <button className="theme-toggle" onClick={() => setTheme((value) => value === 'dark' ? 'light' : 'dark')} aria-label={theme === 'dark' ? '切换浅色主题' : '切换深色主题'} title={theme === 'dark' ? '浅色主题' : '深色主题'}>{theme === 'dark' ? <Sun /> : <Moon />}</button>}
...
export default function App() {
  return <div className="site-root"><SiteHeader /><Routes>
