...
const AUTHORED_LEVELS: readonly ElectromagneticLevel[] = [
  ...
  level({
    ...
    referenceSolution: [charge('参考正电荷', 'positive-charge', 7, 9, 2.2)],
  }),
  level({
    ...
    referenceSolution: [charge('参考正电荷', 'positive-charge', 7.2, 8.8, 1.9)], maxSimulationTime: 5,
  }),

  level({
    id: 7, title: '推拉协奏', subtitle: '中级 · 吸引与排斥', difficulty: 'intermediate',
    briefing: '同时利用上方排斥和下方吸引。', hint: '两个力可以指向同一侧。',
    particles: [particle('p⁺', { x: 1, y: 5 }, { x: 4.3, y: 0 })], obstacles: [circle('中央障碍', 9, 4.8, 1.15)], collectors: [collector('下出口', 16, 9, 1.1)],
  ...
  level({
    ...
    particles: [particle('e⁻', { x: 1, y: 5 }, { x: 4.3, y: 0 }, -1, 1, '#ff8fb3')], obstacles: [circle('中央障碍', 9, 4.8, 1.15)], collectors: [collector('下出口', 16, 9, 1.1)],
    availableTools: ['positive-charge', 'negative-charge'], placementLimits: { 'positive-charge': 1, 'negative-charge': 1 },
  ...
  level({
    ...
    availableTools: ['positive-charge'], placementLimits: { 'positive-charge': 2 }, referenceSolution: [charge('转向一', 'positive-charge', 5.4, 3.5, 0.85), charge('转向二', 'positive-charge', 10.5, 8.5, 0.85)],
  }),
  level({
