...
const REINFORCEMENT_SECONDS = [10, 30, 60, 90, 120, 150] as const
const DETONATION_SECONDS = [30, 60, 90, 120, 150] as const

type StatusFlags = Record<'poison' | 'burn' | 'freeze' | 'radiation' | 'armorBreak' | 'vulnerable', boolean>

const statusFlags = (status: EnemyStatus): StatusFlags => ({
...
function routeSamples(level: LevelConfig, routeIndex: number, count = 160) {
  const path = level.paths?.[routeIndex] ?? level.path
  return Array.from({ length: count + 1 }, (_, index) => pointOnPath(path, index / count))
}

function rankedTerminalHoles(level: LevelConfig, state: BattleState, kind: DeviceKind, bias: RouteBias) {
...
function distanceToRoutes(level: LevelConfig, point: { x: number; y: number }) {
...
function sourceCandidates(level: LevelConfig, state: BattleState, targetId: string) {
...
function addSourceToTarget(state: BattleState, level: LevelConfig, kind: SourceKind, targetId: string) {
...
function upgradeTo(state: BattleState, placementId: string, targetLevel: 1 | 2 | 3) {
  let next = state
...
function preferredReinforcementKinds(level: LevelConfig): DeviceKind[] {
  if (level.id <= 3) return ['bulb', 'laser-emitter']
...
function reinforce(state: BattleState, level: LevelConfig, reinforcementIndex: number) {
  let next = state
  let spentCoins = 0
...
function rankedCapacitorHoles(level: LevelConfig, state: BattleState) {
  const occupied = new Set(state.placements.filter((item) => !item.destroyed).map((item) => item.holeId))
...
function addCollectorCapacitor(state: BattleState, level: LevelConfig) {
...
function augmentRecommended(state: BattleState, level: LevelConfig) {
...
function buildLevelOne(state: BattleState, level: LevelConfig) {
...
function buildRecommended(level: LevelConfig) {
  let state = createBattleState(level)
  if (level.id === 1) return buildLevelOne(state, level)
  if (level.id <= 4) {
    if (level.id === 4) {
      state = addSplitBranch(state, level, 'source-red', 'bulb')
      state = addSplitBranch(state, level, 'source-red', 'bulb')
    ...
    if (level.id >= 3) {
      ...
      state = addDirectBranch(state, level, 'source-red', 'bulb', 2)
      return addDirectBranch(state, level, 'source-red', 'bulb', 2)
    }
    state = addDirectBranch(state, level, 'source-red', 'bulb', 2)
    state = addDirectBranch(state, level, 'source-red', 'bulb', 2)
    return state
  ...
  if (level.id === 6) {
    state = addRgbPrismNetwork(state, level)
    for (const terminal of state.placements.filter((placement) => terminalAttackRange(placement) > 0)) {
      const upgraded = upgradeDevice(state, terminal.id)
      if (upgraded.ok) state = upgraded.state
    }
    return state
  }
  if (level.id <= 8) {
    ...
    state = addDirectBranch(state, level, 'source-red', 'laser-emitter', level.id === 7 ? 2 : 1)
    if (level.id === 8) state = addDirectBranch(state, level, 'source-red', 'bulb', 1)
    return state
  }
  if (level.id <= 11) {
    state = addWhiteBranch(state, level, 'bulb', 3)
    state = addDirectBranch(state, level, level.id === 9 ? 'source-green' : 'source-blue', level.id === 9 ? 'brazier' : 'frost-tower', 3)
    return state
  }
  if (level.id === 12) {
    state = addWhiteBranch(state, level, 'bulb', 3)
    state = addDirectBranch(state, level, 'source-green', 'brazier', 2)
    return addDirectBranch(state, level, 'source-green', 'laser-emitter', 2)
  }
  if (level.id === 13) {
  ...
  if (level.id === 14) {
    ...
    state = addDirectBranch(state, level, 'source-red', 'laser-emitter', 3, 0)
    return addDirectBranch(state, level, 'source-red', 'laser-emitter', 3, 1)
  }
  if (level.id === 15) {
    state = addWhiteBranch(state, level, 'bulb', 3)
    state = addDirectBranch(state, level, 'source-blue', 'frost-tower', 3)
  ...
  if (level.id === 18) {
    state = addWhiteBranch(state, level, 'bulb', 3, 0)
    state = addWhiteBranch(state, level, 'bulb', 3, 1)
    return addDirectBranch(state, level, 'source-green', 'brazier', 3)
  }
  state = addWhiteBranch(state, level, 'bulb', 3)
  state = addWhiteBranch(state, level, 'bulb', 3)
  return addDirectBranch(state, level, 'source-blue', 'frost-tower', 3)
}

function buildWrong(level: LevelConfig) {
...
function simulate(level: LevelConfig, policy: BenchmarkMetrics['policy'], built: BattleState): BenchmarkMetrics {
...
describe('optical defense deterministic campaign benchmark', () => {
  it('runs legal recommended and counter-example policies for all nineteen levels', () => {
    ...
    expect(results[0].recommended).toMatchObject({ phase: 'victory', coreHealth: 12, leaks: 0 })
    expect(results[2].recommended.coreHealth).toBeGreaterThanOrEqual(3)
    expect(results[4].recommended.coreHealth).toBeGreaterThanOrEqual(6)
    const victoryLevelIndexes = [9, 14, 15, 16]
    victoryLevelIndexes.forEach((levelIndex) => expect(results[levelIndex].recommended.phase).toBe('victory'))
    expect(results[16].recommended.coreHealth).toBeGreaterThanOrEqual(2)
    ...
    if (benchmarkReport) {
      console.table(results.flatMap(({ recommended, wrong }) => [recommended, wrong]).map((metrics) => ({
        ...
        kills: metrics.kills,
        leaks: metrics.leaks,
        firstKill: metrics.firstKillSeconds?.toFixed(2) ?? '-',
        status: metrics.statusTriggers,
        statusSamples: metrics.statusSurvivalSamples,
        statusLife: metrics.averageStatusSurvivalSeconds.toFixed(2),
        damageW: metrics.damagePerWatt.toFixed(2),
        damageCoin: metrics.damagePerCoin.toFixed(2),
        power: metrics.peakPowerW,
        capacity: metrics.endingCapacityW,
        coins: metrics.spentCoins,
        capacitor: metrics.capacitorDetonations,
      })))
      console.table(OPTICAL_DEFENSE_LEVELS.map((level) => {
