...
type TrailCache = {
  ...
  key: string
  entries: Map<string, { path: Point[]; drawnIndex: number }>
  lastProgress: number
}
...
/** Return a continuously moving render position without changing the sampled physics path. */
export function sampleParticleRenderFrame(particle: any, isSandbox: boolean, progressState: number): ParticleRenderFrame {
  const path = (particle.path ?? [particle.startPosition ?? particle.position]) as Point[]
  const lastIndex = Math.max(0, path.length - 1)
  if (isSandbox) {
    return {
      path,
      completedIndex: lastIndex,
      position: particle.position ?? path[lastIndex] ?? path[0],
    }
  }
  const cursor = clamp(progressState, 0, 1) * lastIndex
  const completedIndex = Math.min(lastIndex, Math.floor(cursor))
  const nextIndex = Math.min(lastIndex, completedIndex + 1)
  const fraction = nextIndex === completedIndex ? 0 : cursor - completedIndex
  const start = path[completedIndex] ?? particle.startPosition ?? particle.position
...
function isLightCanvasMode() {
  return typeof document !== 'undefined' && document.documentElement.dataset.colorMode === 'light'
}
...
export function ElectromagneticGuideGame({ mode = 'guide' }: ElectromagneticGuideGameProps) {
  ...
  const staticSceneRef = useRef<{ canvas: HTMLCanvasElement; key: string } | null>(null)
  const trailSceneRef = useRef<TrailCache | null>(null)
  const dragRef = useRef<PointerGesture>({ mode: 'idle', pointerId: null, start: { x: 0, y: 0 }, camera: { x: 0, y: 0 }, moved: false, historySaved: false })
  const simulationRef = useRef<any>(simulation)
  const progressRef = useRef(progress)
  const lastProgressUpdateRef = useRef(0)
  const drawFrameRef = useRef<(simulationState: any, progressState: number) => void>(() => {})
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      if (isSandbox) {
        const nextSimulation = stepSimulation(simulationRef.current ?? createSimulation(level), level, dt / 1000, placements as any)
        simulationRef.current = nextSimulation
        drawFrameRef.current(nextSimulation, progressRef.current)
        frame = requestAnimationFrame(tick)
        return
      }
      const nextProgress = Math.min(1, progressRef.current + dt / 3600)
      progressRef.current = nextProgress
      drawFrameRef.current(simulationRef.current, nextProgress)
      if (now - lastProgressUpdateRef.current >= 100) {
        setProgress(nextProgress)
        lastProgressUpdateRef.current = now
      }
      if (nextProgress >= 1) setRunning(false)
      frame = requestAnimationFrame(tick)
    }
    frame = requestAnimationFrame(tick)
    return () => cancelAnimationFrame(frame)
  }, [isDocumentHidden, isSandbox, level, placements, running])

  const run = () => {
    if (isSandbox) {
      if (simulation) setRunning((value) => !value)
      else {
        const initial = createSimulation(level)
        simulationRef.current = initial
        setSimulation(initial); progressRef.current = 1; setProgress(1); setRunning(true)
    ...
    try {
      ...
      simulationRef.current = result; setSimulation(result); progressRef.current = 0; setProgress(0); setRunning(true)
  ...
  const commit = (next: Placement[], text: string) => {
    setHistory((items) => [...items, placements]); setPlacements(next); simulationRef.current = null; setSimulation(null); setRunning(false); progressRef.current = 1; setProgress(1); setMessage(text)
  }
  ...
  useEffect(() => {
    ...
    trailSceneRef.current = null
    const drawStaticScene = (ctx: CanvasRenderingContext2D, view: NonNullable<ReturnType<typeof dimensions>>) => {
      const light = isLightCanvasMode()
      ...
      for (const obstacle of level.obstacles ?? []) {
        ...
        const hazard = ctx.createLinearGradient(p.x - grid, p.y - grid, p.x + grid, p.y + grid); hazard.addColorStop(0, '#712e38'); hazard.addColorStop(.48, '#2a1a23'); hazard.addColorStop(1, '#8d3940'); ctx.fillStyle = light ? '#b84e58' : hazard
        if (obstacle.shape === 'circle') {
          ...
          ctx.strokeStyle = light ? 'rgba(255,221,188,.62)' : 'rgba(255,205,165,.38)'; ctx.beginPath(); ctx.arc(p.x - radius * .14, p.y - radius * .14, radius * .68, Math.PI * 1.05, Math.PI * 1.72); ctx.stroke()
          ctx.fillStyle = light ? '#622a32' : '#130d12'; ctx.beginPath(); ctx.arc(p.x, p.y, radius * .24, 0, Math.PI * 2); ctx.fill()
        } else {
          const size = obstacle.size; const x = p.x - size.x * grid / 2; const y = p.y - size.y * grid / 2; const w = size.x * grid; const h = size.y * grid
          ctx.fillRect(x, y, w, h); ctx.strokeRect(x, y, w, h); ctx.shadowBlur = 0; ctx.save(); ctx.beginPath(); ctx.rect(x, y, w, h); ctx.clip(); ctx.strokeStyle = light ? 'rgba(255,222,166,.62)' : 'rgba(255,190,125,.28)'; ctx.lineWidth = clamp(grid * .12, 4, 8)
          ...
          ctx.restore(); ctx.fillStyle = light ? '#8e3948' : '#c76c6f'; for (const bx of [x + 5, x + w - 5]) for (const by of [y + 5, y + h - 5]) { ctx.beginPath(); ctx.arc(bx, by, 1.8, 0, Math.PI * 2); ctx.fill() }
        }
        ctx.restore()
      }
    ...
    const drawFrame = (simulationState: any, progressState: number) => {
      const canvas = canvasRef.current
      const ctx = canvas?.getContext('2d')
      const view = dimensions()
      if (!canvas || !ctx || !view) return
      const cacheKey = [canvas.width, canvas.height, view.dpr, view.origin.x.toFixed(3), view.origin.y.toFixed(3), view.scale.toFixed(3), isSandbox, level.id, selectedPlacementId, isLightCanvasMode() ? 'light' : 'dark'].join('|')
      let cached = staticSceneRef.current
      if (!cached || cached.key !== cacheKey || cached.canvas.width !== canvas.width || cached.canvas.height !== canvas.height) {
        const staticCanvas = cached?.canvas ?? document.createElement('canvas')
        staticCanvas.width = canvas.width; staticCanvas.height = canvas.height
        const staticCtx = staticCanvas.getContext('2d')
        if (staticCtx) drawStaticScene(staticCtx, view)
        cached = { canvas: staticCanvas, key: cacheKey }
        staticSceneRef.current = cached
      }
      const grid = view.scale
      ctx.setTransform(view.dpr, 0, 0, view.dpr, 0, 0); ctx.clearRect(0, 0, view.rect.width, view.rect.height)
      if (cached?.canvas) ctx.drawImage(cached.canvas, 0, 0, view.rect.width, view.rect.height)
      const states = simulationState?.particles ?? simulationState?.particleStates ?? []
      const particles = states.length ? states : level.particles ?? []
      const renderFrames = particles.map((particle: any) => ({
        particle,
        frame: sampleParticleRenderFrame(particle, isSandbox, progressState),
      }))
      let trail = trailSceneRef.current
      const progressRestarted = !isSandbox && trail && progressState + 1e-6 < trail.lastProgress
      const pathRestarted = trail && renderFrames.some(({ particle, frame }: any) => {
        const entry = trail?.entries.get(particle.id)
        return entry && frame.completedIndex < entry.drawnIndex
      })
      if (!trail || trail.key !== cacheKey || trail.canvas.width !== canvas.width || trail.canvas.height !== canvas.height || progressRestarted || pathRestarted) {
        const trailCanvas = trail?.canvas ?? document.createElement('canvas')
        trailCanvas.width = canvas.width; trailCanvas.height = canvas.height
        trail = { canvas: trailCanvas, key: cacheKey, entries: new Map(), lastProgress: progressState }
        trailSceneRef.current = trail
      }
      const trailCtx = trail.canvas.getContext('2d')
      if (trailCtx) {
        trailCtx.setTransform(view.dpr, 0, 0, view.dpr, 0, 0)
        for (const { particle, frame } of renderFrames) {
          let entry = trail.entries.get(particle.id)
          if (!entry) {
            entry = { path: frame.path, drawnIndex: 0 }
            trail.entries.set(particle.id, entry)
          }
          if (frame.completedIndex > entry.drawnIndex) {
            const startIndex = Math.min(entry.drawnIndex, frame.path.length - 1)
            const start = toScreen(frame.path[startIndex], view)
            trailCtx.beginPath(); trailCtx.moveTo(start.x, start.y)
            for (let index = startIndex + 1; index <= frame.completedIndex; index++) {
      ...
      trail.lastProgress = progressState
      ctx.drawImage(trail.canvas, 0, 0, view.rect.width, view.rect.height)
      for (const { particle, frame } of renderFrames) {
        const pathStart = frame.path[frame.completedIndex]
        if (pathStart && (pathStart.x !== frame.position.x || pathStart.y !== frame.position.y)) {
    ...
    drawFrame(simulationRef.current, progressRef.current)
    return () => {
      if (drawFrameRef.current === drawFrame) drawFrameRef.current = () => {}
    }
  }, [allPlacements, dimensions, isSandbox, level, selectedPlacementId, toScreen, world.height, world.width])

  useEffect(() => {
    const observer = new MutationObserver(() => {
      staticSceneRef.current = null
      trailSceneRef.current = null
      drawFrameRef.current(simulationRef.current, progressRef.current)
    })
  ...
  useEffect(() => {
    const canvas = canvasRef.current
    if (!canvas) return
    const redraw = () => drawFrameRef.current(simulationRef.current, progressRef.current)
  ...
  const onPointerMove = (event: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    simulationRef.current = null; setSimulation(null); setRunning(false); progressRef.current = 1; setProgress(1)
  }
  ...
  const updateSelectedPlacement = (patch: Partial<Placement>, text = '参数已更新，重新发射即可观察变化。') => {
    ...
    simulationRef.current = null; setSimulation(null); setRunning(false); progressRef.current = 1; setProgress(1); setMessage(text)
  }
